...
const PATIENT_TITLES = ["Mr.", "Mrs.", "Ms.", "Dr."];
...
export default function CreatePatientPage({ onNavigate }) {
  const [form, setForm] = useState({
    title: "", first_name: "", last_name: "",
    date_of_birth: "", taj_number: "",
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setSuccess(true);
        setForm({ title: "", first_name: "", last_name: "", date_of_birth: "", taj_number: "" });
      } else {
  ...
  if (success) {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flex: 1, gap: 16 }}>
        ...
        <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#1e3a8a" }}>Patient created successfully</h2>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1, padding: "40px 16px" }}>
      <div style={{ width: "100%", maxWidth: 480 }}>
        <h2 style={{ margin: "0 0 4px", fontSize: 22, fontWeight: 800, color: "#1e3a8a" }}>Add Patient</h2>
        <p style={{ margin: "0 0 28px", color: "#64748b", fontSize: 14 }}>
          Register a new patient to use in consultations.
        </p>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          >
            ...
            {PATIENT_TITLES.map(t => <option key={t}>{t}</option>)}
          </select>
        </div>
        ...
        <div style={{ marginBottom: 24 }}>
          <label style={labelStyle}>TAJ number</label>
          <input
            style={inputStyle("taj_number")}
            value={form.taj_number}
            onChange={set("taj_number")}
